...
```
用户（企业微信）→ Kong API Gateway → Agent 容器 → 技能调用 → ERP/OA/LLM API
                                          ↕
                                  KPI 考核员 Agent
                                          ↕
                                  KPI 数据库 / 仪表盘
...
```
Layer 1: Kong AI Audit Log → 记录 LLM 推理相关
Layer 2: Kong 通用日志 → 记录所有 API 调用（ERP/OA/LLM 全量）
```

![数字员工日志架构](/assets/images/2026-05-09/mfg-digital-worker-03-log-architecture.png)
...
```
┌─────────────┐    ┌──────────────────┐    ┌─────────────┐
│  Kong 日志   │───▶│  KPI 考核员 Agent  │───▶│  KPI 数据库  │
│  (Loki/ES)  │    │  (定时任务触发)    │    │  (PostgreSQL)│
└─────────────┘    └──────────────────┘    └──────┬──────┘
                                                   │
┌─────────────┐    ┌──────────────────┐           │
│  ERP 修正记录 │───▶│  数据采集技能     │           │
...
**《制造业数字员工建设（四）：实战案例与大中型企业落地经验》**
- 100 人小厂 vs 3000 人大厂的数字员工建设对比
- 数字员工"离职"怎么办？（知识导出、权限回收、资产转移）
- 多工厂、多地域数字员工协同方案
- 让数字员工自主修复自身 Bug——Agent Self-Healing 实践
- 5 个真实案例的数字员工投入产出比（ROI）分析

**建议收藏本文**，系列持续更新中，关注不迷路！

---

*本文是「制造业数字员工建设」系列第三篇，后续将深入实战案例与落地经验。欢迎关注掘金/知乎专栏或留言交流您的数字员工建设经验。*
